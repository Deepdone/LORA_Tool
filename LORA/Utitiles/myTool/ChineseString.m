...
+(NSMutableArray*)LetterSortArray:(NSArray*)stringArr
{
    ...
    for (NSObject* object in tempArray) {
        NSString *pinyin = [((ChineseString*)object).pinYin substringToIndex:1];
        FoodSubSorteModel *subSorteModel = ((ChineseString*)object).subSorteModel;
        //不同
        if(![tempString isEqualToString:pinyin])
        {
            //分组
            item = [NSMutableArray array];
            [item  addObject:subSorteModel];
...
//        FoodSubSorteModel *subSorteModel = [stringArr objectAtIndex:i];
//        chineseString.subSorteModel      = subSorteModel;
//        chineseString.string             = [NSString stringWithString:subSorteModel.name];
